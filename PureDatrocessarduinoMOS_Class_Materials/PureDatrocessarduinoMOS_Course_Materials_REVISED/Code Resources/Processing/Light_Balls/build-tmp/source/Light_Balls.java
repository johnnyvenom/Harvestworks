...
public class Light_Balls extends PApplet {
...
public void makeByte_txByte(){
  //clear out the cmd byte
  txByte = 0;
  //here we use the << shift left and | operators to create a byte from bits
  txByte = PApplet.parseByte(PApplet.parseInt(balls[0].state) | PApplet.parseInt(balls[1].state) << 1 | PApplet.parseInt(balls[2].state) << 2 |PApplet.parseInt(balls[3].state) << 3); 
...
class Ball{
  ...
  static public void main(String[] passedArgs) {
    String[] appletArgs = new String[] { "Light_Balls" };
    if (passedArgs != null) {
